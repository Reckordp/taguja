class PabrikFiturBot
  LESS_PRIVILEGE = <<PESAN
Kamu tidak diperbolehkan menggunakan fitur *.%s*
Gunakan *.prem* untuk memakai fitur
PESAN
  ...
  def seleksi_fitur(perintah)
    ...
    jawaban = fitur_terseleksi.collect do |ft|
      next (LESS_PRIVILEGE % perintah[:nama]) unless memenuhi?(c, ft)
      ft.lakukan(perintah)
...
module TagujaManajer
  ...
  @kendali_plugin = PluginManajer.new
  @keterangan = nil
  @seragam = nil
  ...
  def self.memulai
    ...
    Taguja.balikjs do |plugin|
      TagujaManajer.plugin.tambahkan(plugin)
    end
  end
