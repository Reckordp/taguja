...
void Realm::BootstrapTaguja() {
  Taguja* penyedia = env_->taguja();
  penyedia->instance = GetModuleHandle(NULL);
  JendelaInit(reinterpret_cast<HINSTANCE*>(&penyedia->instance));
  RenderInit(reinterpret_cast<HINSTANCE*>(&penyedia->instance));
  penyedia->Masuk();
